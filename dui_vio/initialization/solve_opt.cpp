...
bool OptSolver::solveHybrid(const vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &Rji, Vector3d &tji, vector<Vector3d>* pcov)
{
...
bool OptSolver::solveTCeres(const vector<pair<Vector3d, Vector3d>> &corres, const Matrix3d &Rji, Vector3d &tji, vector<Vector3d>* pcov)
{
	para_T[0][0] = tji(0); 
	para_T[0][1] = tji(1); 
	para_T[0][2] = tji(2); 
    ...
    for(int i=0; i<corres.size(); i++){
    	...
    	if(pcov != NULL){
    		sigma_pt3d(cov_pti, (*pcov)[i].x(), (*pcov)[i].y(), pti.z(), (*pcov)[i].z());
    	}
